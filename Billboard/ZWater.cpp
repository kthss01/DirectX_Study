...
// 4각형 정점버퍼를 만든다.
// 지금은 4각형 메시를 여러번 출력하는데, 이것은 비효율적인 방식
// 물전체를 표현할 수 있는 커다란 정점버퍼(혹은 메시)를 만드는것이 더 낫다
int ZWater::_CreateVIB()
{
	...
	if (FAILED(m_pDev->CreateVertexBuffer(
		4 * sizeof(WATERVERTEX), 0, WATERVERTEX::FVF,
...
int ZWater::_WaveTexture()
{
	...
	if ((nCurrTick - m_nPrevTick) > m_nWaveTick) {
		...
		// 0번 스테이지의 텍스처는 텍스처변호나 행렬(D3DTS_TEXTURE0)과 곱해짐
		m_pDev->SetTextureStageState(0, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);
		// 1번 스테이지의 텍스처는 텍스처변호나 행렬(D3DTS_TEXTURE1)과 곱해짐
...
ZWater::~ZWater()
{
	SAFE_RELEASE(m_pTexWater[0]);
	SAFE_RELEASE(m_pTexWater[1]);
	SAFE_RELEASE(m_pVB);
	SAFE_RELEASE(m_pIB);
}
...
int ZWater::Draw()
{
	...
	// 루프를 돌며 메시 그리기
	// 비효율적임
	for (z = 0.0f; z < (float)m_cz; z+=1.0f) {
		for (x = 0.0f; x < (float)m_cx; x+=1.0f) {
